...
@section('title', __('My Hotel Details: :name', ['name' => $hotel->name]))

@section('content')
<div class="py-6">
    <div class="max-w-5xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">{{ __('My Hotel Details') }}</h1>
            <a href="{{ route('hotel_admin.panel.hotel.edit') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 shadow-sm whitespace-nowrap">
        ...
        <div class="mt-8">
            <div class="flex justify-between items-center px-4 py-5 sm:px-6 border-b border-gray-200 bg-white shadow-md sm:rounded-t-lg">
                ...
                <a href="{{ route('hotel_admin.panel.rooms.create') }}" class="px-3 py-1 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 text-sm flex items-center">
                        ...
                        @forelse($hotel->rooms as $room)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <a href="{{ route('hotel_admin.panel.rooms.show', $room->room_id) }}" class="text-blue-600 hover:text-blue-900 transition duration-150 ease-in-out ml-2">
                                        {{ __('View') }}
                                    </a>
                                    <a href="{{ route('hotel_admin.panel.rooms.edit', $room->room_id) }}" class="text-indigo-600 hover:text-indigo-900 transition duration-150 ease-in-out ml-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                                            <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('hotel_admin.panel.rooms.destroy', $room->room_id) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('Are you sure you want to delete this room?') }}');">
                                        @csrf
                                        @method('DELETE')
