...
@section('title', __('Edit My Hotel Information'))
...
<div class="py-6">
    <div class="max-w-3xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('hotel_admin.panel.hotel.show') }}" class="text-indigo-600 hover:text-indigo-800 transition duration-150 ease-in-out">
                → {{ __('Back to My Hotel Details') }}
            </a>
        </div>

        <div class="bg-white shadow-md px-6 py-8 sm:rounded-lg">
            <form action="{{ route('hotel_admin.panel.hotel.update') }}" method="POST">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- Hidden admin_user_id as it's implicit --}}
                    <input type="hidden" name="admin_user_id" value="{{ $hotel->admin_user_id }}">
                    <div class="md:col-span-2">
                ...
                <div class="pt-8 flex justify-end">
                    <a href="{{ route('hotel_admin.panel.hotel.show') }}" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        {{ __('Cancel') }}
                    </a>
